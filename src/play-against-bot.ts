...
async function main() {
  const gameLoop = new GameLoop(['human', 'bot']);

  // Add a human agent for player 1
  const humanAgent = new HumanAgent('human');
  gameLoop.addAgent('player-1', humanAgent);

  // Add a simple agent for player 2
