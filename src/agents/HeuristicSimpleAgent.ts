...
export class HeuristicSimpleAgent extends ExhaustiveSimpleAgent {
  ...
  makeMove(snapshot: GameSnapshot, playerId: string): Promise<Card | null> {
    ...
    if (DEBUG_TIMING) {
      ...
      logger.info(
        `[HeuristicSimpleAgent.makeMove] ${totalDuration}ms total (parse: ${parseDuration}ms, filter: ${filterDuration}ms, scoring: ${scoringDuration}ms, select: ${selectDuration}ms)`
      );
      logger.info(
        `  - Valid cards: ${validPlayedCards.length}, Chose: ${bestCard.card} (score: ${bestCard.score}, heuristic: ${bestCard.heuristic})`
      );
    }
